...
class _DoneState extends State<Done> {
 ...
 @override
  ...
  Future<void> _setData(String taskName, String taskAbout, DateTime date, String time) async {
    final newTaskRef = widget.dbRef3.push();
  ...
  Future<void> _updateData(taskKey, taskName, taskAbout, date, time) async {
    await widget.dbRef3.child(taskKey).update({
      "name": "$taskName",
      "about": "$taskAbout",
      "date": "$date",
      "time": "$time",
  ...
  Future<void> _getData() async {
    final snapshot = await widget.dbRef3.child('').get();
    if (snapshot.exists) {
      setState(() {
  ...
  Future<void> _deleteData(taskKey) async {
    await widget.dbRef3.child(taskKey).remove();
  }

  Future<void> _moveLeftData(taskKey, taskName, taskAbout, date, time) async {
    await widget.dbRef3.child(taskKey).get();
                ...
                return Dismissible(
                  ...
                  onDismissed: (direction) {
                    if (direction == DismissDirection.startToEnd) {
                      ...
                      _moveLeftData(key.toString(), name, about, date, time);
                    }
                  },
                  background: Container(
                    alignment: Alignment.centerLeft,
                    padding: const EdgeInsets.only(left: 16.0),
                    child: Icon(Icons.north, color: Theme.of(context).colorScheme.onBackground, size: 30,),
                  ),
