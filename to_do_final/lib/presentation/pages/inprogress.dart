...
class InProgress extends StatefulWidget {
  ...
  final FirebaseDatabase database3 = FirebaseDatabase.instanceFor(app: Firebase.app('third DB'));
  final DatabaseReference dbRef3 = FirebaseDatabase.instanceFor(app: Firebase.app('third DB')).ref().child('doneuser');

  final FirebaseDatabase database4 = FirebaseDatabase.instanceFor(app: Firebase.app('forth DB'));
  final DatabaseReference dbRef4 = FirebaseDatabase.instanceFor(app: Firebase.app('forth DB')).ref().child('declineduser');
...
class _InProgressState extends State<InProgress> {
  ...
  Future<void> _setData(String taskName, String taskAbout, DateTime date, String time) async {
    final newTaskRef = widget.dbRef2.push();
  ...
  Future<void> _updateData(taskKey, taskName, taskAbout, date, time) async {
    String formattedDate = DateFormat('yyyy-MM-dd').format(date);
    await widget.dbRef2.child(taskKey).update({
      "name": "$taskName",
      "about": "$taskAbout",
      "date": "$formattedDate",
      "time": "$time",
  ...
  Future<void> _getData() async {
    final snapshot = await widget.dbRef2.child('').get();
    if (snapshot.exists) {
      setState(() {
  ...
  Future<void> _deleteData(taskKey) async {
    await widget.dbRef2.child(taskKey).remove();
  }

  Future<void> _moveLeftData(taskKey, taskName, taskAbout, date, time) async {
    await widget.dbRef2.child(taskKey).get();
    final moveRef = widget.dbRef1.push();
    await moveRef.set({
      "name": taskName,
      "about": taskAbout,
      "date": date,
  ...
  Future<void> _moveRightData(taskKey, taskName, taskAbout, date, time) async {
    await widget.dbRef2.child(taskKey).get();
    final moveRef = widget.dbRef3.push();
    await moveRef.set({
      "name": taskName,
      "about": taskAbout,
      "date": date,
                ...
                return Dismissible(
                  ...
                  onDismissed: (direction) async {
                    if (direction == DismissDirection.startToEnd) {
                      // Змах вправо
                     await _moveRightData(key.toString(), name, about, date, time);
                    } else if (direction == DismissDirection.endToStart) {
                      // Змах вліво
                     await  _moveLeftData(key.toString(), name, about, date, time);
                    }
                  },
                  background: Container(
                    alignment: Alignment.centerLeft,
                    padding: const EdgeInsets.only(left: 16.0),
                    child: Icon(Icons.done, color: Theme.of(context).colorScheme.onBackground, size: 30,),
                  ),
                  secondaryBackground: Container(
                    alignment: Alignment.centerRight,
                    padding: const EdgeInsets.only(right: 16.0),
                    child: Icon(Icons.list, color: Theme.of(context).colorScheme.onBackground, size: 30,),
                  ),
                  child: Card(
